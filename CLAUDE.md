...
- **Botón "Ver ejemplo" (`SNAPSHOT`) queda EXENTO de este bloqueo**: como `loadSnapshot()` no pasa
  por `recompute()` sino que llama directo a las mismas funciones compartidas (`renderComboChartWithMonths`,
  `renderTable`, `renderMetasVendedores`, `renderRankTable`), el bloqueo de mes viviría "gratis"
  también ahí — pero `SNAPSHOT` es un dataset fijo de julio 2026, no datos de "hoy", así que
  restringirlo rompería el propósito ilustrativo del ejemplo fuera de julio. Se agregó la bandera
  `let vistaEjemploActiva = false;` (junto a `adminUnlocked`): se pone en `true` al inicio de
  `loadSnapshot()`. Todas las guardas del bloqueo usan `!adminUnlocked && !vistaEjemploActiva`, así
  que "Ver ejemplo" sigue mostrando pastillas/selects/ranking completo para cualquiera, igual que
  antes de este cambio. **La bandera se resetea a `false` al inicio de `recompute()`** (que siempre
  trabaja con datos reales del `store`, nunca con `SNAPSHOT`) — esto cubre los tres caminos que
  podían dejarla "pegada" en `true` dentro de la misma sesión (login admin, logout admin, o subir
  un excel real después de ver el ejemplo), detectado en la auditoría de este cambio.
