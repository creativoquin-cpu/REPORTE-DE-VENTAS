...
- KPIs de EFFI y Dropi siempre separados **para el administrador**. Para un visitante sin la clave, en cambio, varias piezas del reporte se muestran ya combinadas (ver "Vista visitante bloqueada al mes actual" más abajo: KPIs ocultos + tarjeta "Ventas totales (mes)", ranking de tiendas oculto, "Detalle por día" y "Comparativo diario" con una sola serie EFFI+Dropi). Para el administrador, las excepciones explícitas donde EFFI y Dropi sí se cruzan siguen siendo la meta combinada del día operativo (ver abajo) y la tarjeta KPI "Total combinado (último día)" (`kpiCombinadoTotal`) — muestra **solo el último día operativo con datos** (`dates[dates.length-1]`), NO la suma de todo el rango subido, junto con el badge de si cumplió o no la meta de 200 ese día.
...
- Los festivos de Colombia se calculan en vivo con el algoritmo de Gauss/Meeus para la Pascua (`easterSundayDate()`), válido para cualquier año — no es una lista fija que haya que mantener a mano. Incluye fechas fijas (1 ene, 1 may, 20 jul, 7 ago, 8 dic, 25 dic), las que la Ley Emiliani traslada al lunes siguiente (Reyes, San José, San Pedro y San Pablo, Día de Nuestra Señora del Rosario de Chiquinquirá —festivo nuevo agregado por la Ley 2578 de 2026—, Asunción, Día de la Raza, Todos los Santos, Independencia de Cartagena), y las que dependen de la Pascua (Jueves y Viernes Santo, Ascensión, Corpus Christi, Sagrado Corazón).
- El reparto se hace en unidades **enteras**, sin decimales: `repartirEntero(total, n)` calcula `piso = Math.floor(total/n)` para todos los días de la ventana y reparte el residuo (`total - piso*n`) sumando +1 a los **últimos** días de la ventana (los miembros vienen en orden cronológico ascendente, así que "últimos" = los días más recientes). Ej. 8 prendas entre 3 días (sábado+domingo+lunes festivo) → piso=2, resto=2 → 2, 3, 3 (no 2,6667 cada uno). Así la suma mostrada siempre cierra exacto con el total real de la ventana, sin depender de redondear cada día por separado. `repartirFinDeSemanaVendedores()` usa el mismo `repartirEntero()` pero por vendedor de forma independiente — en teoría el total de un día sumando todos los vendedores puede diferir en ±1 unidad del valor que da `repartirFinDeSemana()` para ese mismo día (matemáticamente esperable con floor+residuo no lineal repartido por separado); hoy no es visible porque el panel que lo usa ("Cumplimiento de vendedores") está oculto.
...
Un visitante sin la clave (`adminUnlocked === false`) ya no puede navegar a meses anteriores en
ninguna parte del reporte: todo queda fijo en el **mes calendario actual** (`todayMonthKey()`,
según el reloj del sistema — no el mes más reciente con datos). El administrador conserva
navegación libre de mes en todo, sin cambios.

- **Tarjetas KPI** (Top vendedor/tienda, "Total combinado (último día)", "Promedio diario (mes)"):
  ya se filtraban por `comboSelectedMonth` (ver más abajo); heredan el bloqueo automáticamente.
- **"Comparativo diario"**: `resolveComboMonth(dates)` fuerza `comboSelectedMonth = todayMonthKey()`
  en CADA llamada (no solo la primera vez) para visitantes, e incluye ese mes en la lista aunque no
  tenga datos aún. `renderComboMonthPills()` vacía y oculta (`display:none`) el contenedor
  `#comboMesPills` para visitantes — no hay pastillas que clickear. Para admin, ambas funciones se
  comportan exactamente igual que antes (default solo la primera vez, pastillas visibles).
- **"Detalle por día"** (`detalleMesSelect`, en `renderTable()`) y **"Cumplimiento de vendedores"**
  (`metasMesSelect`, en `renderMetasVendedores()`): para visitantes, el `<select>` se sigue
  poblando con `populateMonthSelect()` pero se fuerza `select.value = todayMonthKey()` y se oculta
  (`display:none`) — como ambas funciones ya deciden qué renderizar leyendo `select.value`, al
  forzarlo caen automáticamente en la rama "detalle día a día" del mes actual; un visitante nunca
  ve la vista "Resumen mensual" (que mostraría el histórico completo). Para admin, comportamiento
  sin cambios (select visible, respeta la selección previa, incluye "Resumen mensual").
- **"Ranking de vendedores" / "Ranking de tiendas"** (`rankVendedorBody`/`rankTiendaBody`): antes
  mostraban siempre `rankA`/`rankB` (todo el histórico) para cualquiera. Ahora, en `recompute()`,
  `adminUnlocked ? rankA : rankAMes` (mismo patrón para tiendas con `rankB`/`rankBMes`) — el
  visitante ve solo el ranking acumulado del mes actual, reutilizando `rankAMes`/`rankBMes` (ya
  calculados para las tarjetas KPI "Top vendedor"/"Top tienda"). El admin sigue viendo el ranking
  completo de siempre, sin cambios.
- **"Historial y comparativo mensual"**: la sección completa (gráfico + tabla + su
  `historialMesSelect`) ahora es **admin-only** — se ocultó por completo para visitantes. El
  `<section>` que la envuelve recibió `id="historialSection"` y la clase `admin-content`
  (reutiliza el CSS ya existente `.admin-content{display:none}`, el mismo patrón que ya ocultaba
  `#uploadZoneWrap`/`#adminContent`). `tryUnlockAdmin()` la muestra (`display:'block'`) junto con
  las otras dos zonas; el botón "Cerrar modo administrador" (`adminLockBtn`) la vuelve a ocultar.
- **KPIs exclusivos de administrador + tarjeta combinada para visitante**: un visitante sin la
  clave no ve "Unidades EFFI (mes)" (`kpiCardEffiMes`), "Unidades Dropi (mes)" (`kpiCardDropiMes`)
  ni "Top tienda Dropi (mes)" (`kpiCardTopTienda`) por separado — esas tres tarjetas quedan con
  `style.display='none'`, aplicado en `recompute()` según `adminUnlocked`. En su lugar, entre
  "Unidades Dropi (mes)" y "Total combinado (último día)" hay una tarjeta nueva, siempre visible
  para cualquiera, "Ventas totales (mes)" (`kpiVentasTotalesMes`) — reutiliza `sumPropias+sumTotal`
  (las mismas sumas ya calculadas para las tarjetas EFFI/Dropi del mes elegido). El administrador
  sigue viendo las 6 tarjetas originales, más esta séptima.
- **Ranking de tiendas oculto para visitante**: el panel "Ranking de tiendas" (`rankTiendaPanel`,
  dentro de `#rankGrid`) se oculta por completo con `style.display='none'` si `!adminUnlocked`, y
  `#rankGrid` recibe la clase `.rank-grid.single` (una sola columna) para que "Ranking de
  vendedores" ocupe el ancho completo sin dejar un hueco. La tabla `rankTiendaBody` se sigue
  llenando por dentro aunque esté oculta, sin efecto visible.
- **"Detalle por día" y "Comparativo diario" combinados para visitante**: en vez de columnas/series
  separadas de EFFI y Dropi, un visitante ve una sola columna "Unidades totales" (tabla, cabecera
  `tableHeadRow` reescrita dinámicamente en `renderTableBody()`) y una sola serie "Unidades
  totales" (gráfico de línea de `renderCharts()`, sin doble eje; gráfico de barras de
  `renderDetalleChart()`, sin apilar dos series). El título (`comboTitle`) y la leyenda
  (`comboLegend`) de "Comparativo diario" cambian de texto acorde. El administrador sigue viendo
  las dos columnas/series de siempre, sin cambios, en ambos.
...
- **180 prendas por día operativo, evaluado como META DE EQUIPO** (solo EFFI, tras excluir "Cambios"/vacío/"Miguel"). Ya NO se evalúa por vendedor individual: se suma TODO lo que vendió el equipo COMPLETO de EFFI ese día operativo (todos los vendedores juntos) y ESE total se compara contra 180. El badge ✔/✖ de "Cumplimiento de vendedores" es un veredicto por DÍA (equipo), no por vendedor — si Vendedor 1 vendió 3 y Vendedor 2 vendió 5, el total del día es 8 y eso es lo que se compara contra la meta. El desglose de unidades por vendedor sigue visible en la tabla (informativo), pero ya no dispara su propio badge individual. Implementado en `renderMetasBody()`: rama detalle (mes elegido) calcula `teamTotal` por día y pinta el badge una sola vez en la primera fila de ese día (mismo patrón de colapso que la columna "Dia"); rama resumen (agrupado por mes) cuenta `diasCumplidosEquipo` (días donde el total de equipo ≥ 180) en vez de días cumplidos por vendedor. Headers/subtítulo del panel actualizados a "Meta equipo (180)".
- **200 prendas combinadas por día operativo** (EFFI + Dropi sumados). También es una **sumatoria**: total EFFI del día + total Dropi del día. Columna "Meta 200" en "Detalle por día". Esta meta no cambió — sigue siendo combinada EFFI+Dropi, no confundir con la meta de equipo de 180 (que es solo EFFI).
- Constantes `META_VENDEDOR = 180` y `META_COMBINADA = 200` al inicio del script — si el negocio cambia estos números, solo hay que tocar esas dos constantes (el número no cambió con este ajuste, solo cómo se evalúa `META_VENDEDOR`).
- **Panel "Cumplimiento de vendedores" oculto visualmente a pedido del cliente**: su `<div class="panel">` (el que envuelve todo el panel, alrededor de la línea 668 de `reporte-ventas.html`) tiene `style="display:none"` fijo en el HTML, para TODOS los usuarios sin excepción — administrador incluido, no usa `.admin-content` ni ningún toggle. La lógica de cálculo sigue intacta y ejecutándose igual en cada `recompute()` (`META_VENDEDOR=180`, `renderMetasVendedores()`, `renderMetasBody()`, el `<select>` `metasMesSelect` y su listener) — solo no se pinta en pantalla. Es reversible con solo quitar ese `style="display:none"`.

## Vistas

- Comparativo diario: gráfico de línea con dos series (unidades EFFI vs. unidades Dropi por día operativo, ya con el reparto de fin de semana aplicado) **para el administrador**; un visitante sin la clave ve una sola serie combinada "Unidades totales" (EFFI+Dropi sumados, sin doble eje), con el título y la leyenda del panel cambiando de texto acorde (ver "Vista visitante bloqueada al mes actual"). **Solo muestra un mes a la vez** (arranca en el más reciente con datos), con botones tipo pastilla arriba del gráfico (`comboMesPills`) para cambiar de mes con un clic — evita que subir varios meses de histórico deje el gráfico ilegible. Funciones: `renderComboChartWithMonths()`, `renderComboMonthPills()`, `renderComboChartForSelectedMonth()`, `renderCharts()`.
...
- Tabla "Cumplimiento de vendedores" (tope 180) y tabla "Detalle por día" (con meta 200), ambas con columnas EFFI/Dropi separadas **para el administrador**. Un visitante sin la clave ve "Detalle por día" con una sola columna "Unidades totales" (EFFI+Dropi sumados) en vez de dos, tanto en la tabla (`renderTableBody()`, que reescribe la cabecera `tableHeadRow` según el modo) como en el gráfico (`renderDetalleChart()`) — barras EFFI+Dropi apiladas por día con una línea punteada en la meta de 200 para el admin; una sola barra "Unidades totales" con la misma línea de meta para el visitante. Comparte el mismo selector de mes que la tabla (`detalleMesSelect`). Encima de cada columna se dibuja el total del día (EFFI+Dropi) con el mismo plugin `stackedTotalLabelPlugin` que usa "Historial y comparativo mensual" (`barIndices` ajustado a `[0]` en vista combinada o `[0,1]` en vista admin). En modo "Resumen mensual" el gráfico muestra barras por mes sin la línea de meta ni el total encima (la meta es diaria, no mensual).
- **"Detalle por día" duplicado para el administrador (dos paneles en paralelo)**: cuando `adminUnlocked===true`, el reporte muestra DOS paneles "Detalle por día" uno debajo del otro — "Detalle por dia (usuario)" (título de `#detalleTituloUsuario`, siempre combinado EFFI+Dropi en una sola columna/serie, exactamente lo que ve cualquier visitante) y "Detalle por dia (administrador)" (`#detallePanelAdmin`, siempre con EFFI/Dropi en columnas/series separadas). Un visitante sin clave sigue viendo un solo panel, sin sufijo en el título ("Detalle por dia" a secas) — el panel admin nuevo es puramente adicional. Implementado con `DETALLE_PANEL_CONFIG = {usuario:{combinedView:true,...}, admin:{combinedView:false,...}}`, que mapea cada `panelKey` a sus propios ids de DOM (`tableHeadRow`/`tableHeadRowAdmin`, `tableTh1`/`tableTh1Admin`, `tableTh4`/`tableTh4Admin`, `tableBody`/`tableBodyAdmin`, `chartDetalle`/`chartDetalleAdmin`). `renderTable()` sigue con la misma firma de siempre (`renderTable(dates, effiByDate, dropiByDate)`, llamada sin cambios desde `recompute()`) y ahora llama `renderTableBody('usuario')` siempre, y `renderTableBody('admin')` solo si `adminUnlocked`. `renderTableBody(panelKey)` y `renderDetalleChart(panelKey)` reemplazan a las versiones anteriores sin parámetro; `detalleCharts = {usuario:null, admin:null}` reemplaza a la variable única `detalleChart`. Los dos paneles **comparten un único selector de mes** (`detalleMesSelect` — no hay uno propio para el panel admin); su listener `change` ahora llama a `renderTableBody('usuario')` y, si `adminUnlocked`, también a `renderTableBody('admin')`, para que ambos paneles cambien de mes sincronizados con un solo clic. El panel `#detallePanelAdmin` usa el patrón ya existente `.admin-content` (oculto por CSS por defecto) con sus toggles agregados junto a los de `#adminContent`/`#historialSection`: `tryUnlockAdmin()` lo muestra (`display:'block'`) y el listener de `adminLockBtn` lo vuelve a ocultar (`display:'none'`). El comportamiento descrito en "Vista visitante bloqueada al mes actual" para visitantes sin clave no cambió — este panel nuevo es adicional y solo existe con `adminUnlocked===true`.
...
- **Dropi**: 20 unidades totales (24 filas − 4 canceladas, sin deduplicar). 2026-07-09 (jueves) → 1 unidad, sin cambios. 2026-07-10 (viernes) → 11 unidades, sin cambios. 2026-07-11 es **sábado**, con 8 unidades crudas antes del reparto; como el lunes siguiente (2026-07-13) **sí es festivo** (Día de Nuestra Señora del Rosario de Chiquinquirá, Ley 2578 de 2026 — festivo nuevo, ver sección "Reparto de fin de semana/festivo largo"), la ventana se extiende a sábado+domingo+lunes y esas 8 se reparten entre 3 con `repartirEntero(8,3)` (piso=2, resto=2, +1 a los últimos 2 días) → **2026-07-11 → 2**, **2026-07-12 (domingo) → 3** y **2026-07-13 (lunes festivo) → 3** (valores enteros exactos, suma=8, sin redondeo al mostrar). Tienda líder total (no cambia con el reparto, es un total general): "Tiko" con 9 unidades.
- **EFFI**: 99 unidades totales, todas en el día operativo 2026-07-10 (viernes, sin reparto). Vendedora líder: "Lucenith Quintero Leon" con 24 unidades. **Meta de equipo (180)**: el total de EQUIPO ese día es 99 (suma de todos los vendedores) — 99 < 180, badge "no cumplido" (✖, faltan 81). Con esta muestra tan chica el equipo no llega a la meta, es esperable.
...
- **KPI "Total combinado (último día)"** con esta muestra: el último día operativo con datos es 2026-07-13 (lunes festivo, generado por el reparto — antes de la Ley 2578 de 2026 hubiera sido 2026-07-12), EFFI 0 + Dropi 3 = **3 u.** exactos (faltan ~197 para la meta de 200) — no confundir con el combinado de 2026-07-10 (110 u.), que es un día distinto y no el último. Solo aplica en modo visitante (repartido); en modo administrador "real" el último día con datos sigue siendo 2026-07-11 (sábado), porque ese modo usa `A.byDateRaw`/`B.byDateRaw` sin reparto.
...
- **Festivos de Colombia 2026 calculados** (para probar `isFestivoColombia()`): 1 ene, 12 ene (Reyes movido), 23 mar (San José movido), 2 abr (Jueves Santo), 3 abr (Viernes Santo), 1 may, 18 may (Ascensión), 8 jun (Corpus Christi), 15 jun (Sagrado Corazón), 29 jun (San Pedro y San Pablo, cae lunes), 13 jul (Chiquinquirá movido, Ley 2578 de 2026), 20 jul, 7 ago, 17 ago (Asunción movida), 12 oct (Día de la Raza, cae lunes), 2 nov (Todos los Santos movido), 16 nov (Cartagena movido), 8 dic, 25 dic — 19 festivos en total.
