...
- **Panel "Cumplimiento de vendedores" oculto visualmente a pedido del cliente**: su `<div class="panel">` en `reporte-ventas.html` tiene `style="display:none"` fijo, para TODOS los usuarios sin excepción (administrador incluido) — no usa `.admin-content` ni ningún toggle. La lógica sigue intacta y corriendo en cada `recompute()` (`META_VENDEDOR=180`, `renderMetasVendedores()`, `renderMetasBody()`, `metasMesSelect`), solo no se muestra en pantalla. Reversible quitando ese `style`.
