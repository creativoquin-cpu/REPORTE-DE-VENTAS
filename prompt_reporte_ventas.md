...
- El reparto se hace en unidades **enteras**, sin decimales: `repartirEntero(total, n)` calcula `piso = Math.floor(total/n)` para todos los días y suma el residuo (+1) a los **últimos** días de la ventana (orden cronológico ascendente). Ej. 8 entre 3 días (sábado+domingo+lunes festivo) → piso=2, resto=2 → 2, 3, 3 (no 2,6667 cada uno), sin redondeo al mostrar. `repartirFinDeSemanaVendedores()` aplica el mismo `repartirEntero()` por vendedor de forma independiente — el total de un día sumando vendedores puede diferir en ±1 del total que da `repartirFinDeSemana()` para ese día (esperable, floor+residuo no lineal); hoy no visible porque el panel que lo usa está oculto.
...
siguiente 07-13 festivo (Ley 2578 de 2026) la ventana se reparte entre 3 con `repartirEntero(8,3)` y
se ve como 2 + 3 + 3 (07-11/07-12/07-13, valores enteros exactos, sin redondeo) para visitantes, y
como una sola entrada de 8 u. en 07-11 para el admin.
...
- **Meta de equipo EFFI por día operativo, META DE EQUIPO** (solo EFFI, tras excluir "Cambios"/vacío/"Miguel"). Ya NO se evalúa por vendedor individual: se suma TODO lo que vendió el equipo COMPLETO de EFFI ese día (todos los vendedores juntos) y ESE total se compara contra la meta vigente ese día — ejemplo: si Vendedor 1 vendió 3 y Vendedor 2 vendió 5, el total del día es 8, y eso es lo que se compara contra la meta. Se muestra en la tabla **"Cumplimiento de vendedores"** (panel antes de "Detalle por día"): el desglose por vendedor sigue visible (columna Unidades, informativa), pero el badge ✔/✖ ahora se pinta UNA sola vez por día (primera fila de ese día), evaluando `teamTotal` en vez del valor individual. En la vista resumen por mes, se cuenta `diasCumplidosEquipo` (días donde el total de equipo llegó a la meta vigente ESE día) en vez de días cumplidos por vendedor. Header de la columna: "Meta equipo" a secas (ya no muestra un número fijo entre paréntesis, porque la meta ahora puede variar por periodo — ver punto siguiente).
- **La meta de equipo EFFI dejó de ser un número único fijo: ahora es un historial versionado por periodo de vigencia, editable por el administrador**, sin romper ningún resultado ya mostrado hasta que se agregue un periodo nuevo. Estructura: array de periodos `{desde:'YYYY-MM-DD', valor:number}`, ordenado ascendente por `desde`, persistido en `localStorage` bajo la clave `reporteVentasMetasHistorial_v1` (`METAS_VENDEDOR_HISTORIAL_KEY`), vía `loadMetasVendedorHistorial()`/`saveMetasVendedorHistorial()`, cacheado en memoria en `metasVendedorPeriodos` (se inicializa una sola vez al cargar el script, antes de cualquier `recompute()` posterior, incluido el que dispara `loadRawStore()` al final del archivo).
  - Semilla usada la primera vez que se abre la página tras este cambio (si no hay nada guardado todavía en esa clave): un único periodo "desde siempre", `{desde:'2000-01-01', valor:180}` (`METAS_VENDEDOR_SEED`) — así cualquier día operativo ya acumulado (junio/julio 2026, etc.) sigue resolviendo a 180 exactamente igual que antes de este cambio.
  - `metaVendedorVigente(diaISO)` calcula la meta aplicable a un día operativo: el periodo con `desde` más reciente que sea `<= diaISO`. La usan `renderMetasBody()` (las dos ramas, resumen y detalle) y `renderHistorialChart()` (la línea punteada "Meta vendedor" del gráfico día a día, que ahora recalcula el valor por día en vez de repetir una constante).
  - `upsertMetaVendedorPeriodo(desdeISO, valor)` agrega un periodo o, si ya existe uno con esa misma fecha `desde`, sobreescribe su valor **sin duplicar fila**.
  - Un periodo con `desde` futuro no afecta días anteriores a esa fecha; un periodo con `desde` retroactivo dentro de un mes ya transcurrido sí cambia la meta de los días de ese mes desde esa fecha en adelante (permite corregir/ajustar un periodo ya en curso, no solo hacia adelante).
  - **UI nueva dentro del panel** (`#metasPanel`): campos "Vigente desde" (`#metaVendedorDesde`, tipo `date`) y "Meta (prendas/día)" (`#metaVendedorValor`, tipo `number`), botón "Agregar periodo" (`#metaVendedorAddBtn`) que valida fecha y valor entero positivo (errores en `#metaVendedorError`, clases `status-line`/`err`/`ok`), y una tabla de solo lectura (`#metaVendedorPeriodosBody`, pintada por `renderMetaVendedorPeriodosUI()`) con todos los periodos guardados, del más reciente al más antiguo. Guardar un periodo dispara `recompute()` para que el reporte completo (badges y gráfico de Historial) refleje el cambio al instante.
- **200 prendas combinadas por día operativo** (EFFI + Dropi sumados, ambos ya en unidades así que la suma es válida). También es una **sumatoria**: total EFFI del día + total Dropi del día. Se muestra como columna nueva "Meta 200" en la tabla **"Detalle por día"**, mismo tipo de badge. **Esta meta sigue siendo una constante fija, `META_COMBINADA = 200`, sin editor, sin versionado, sin cambios** con este ajuste — no confundir con la meta de equipo EFFI (ahora versionada).
- Constante `META_VENDEDOR = 180` sigue declarada al inicio del script, pero ahora es solo el valor semilla de la migración (`METAS_VENDEDOR_SEED`) y el fallback teórico de `metaVendedorVigente()` — ya NO se usa directamente en `renderMetasBody()` ni en `renderHistorialChart()`, ambos migrados a `metaVendedorVigente(...)`. `META_COMBINADA = 200` sin cambios; si el negocio cambia ese número, solo hay que tocar esa constante.
- **Panel "Cumplimiento de vendedores" reactivado, pero solo para el administrador**: antes su `<div class="panel">` en `reporte-ventas.html` tenía `style="display:none"` fijo para TODOS los usuarios sin excepción. Ahora usa el patrón `.admin-content` (mismo que `#adminContent`/`#historialSection`/`#detallePanelAdmin`): `<div class="panel admin-content" id="metasPanel">`, mostrado por `tryUnlockAdmin()` y ocultado de nuevo por el listener de `#adminLockBtn`, junto con esos otros paneles. Un visitante sin la clave nunca lo ve; el administrador sí, con el editor de periodos incluido.

## Vistas (ya implementadas)

- Comparativo diario: gráfico de línea con dos series (unidades EFFI vs. unidades Dropi por día operativo) **para el administrador**; un visitante sin la clave ve una sola serie combinada "Unidades totales" (EFFI+Dropi sumados, sin doble eje), con el título (`comboTitle`) y la leyenda (`comboLegend`) del panel cambiando de texto acorde. **Solo muestra un mes a la vez** (arranca en el más reciente con datos) — botones tipo pastilla arriba del gráfico (`comboMesPills`) para saltar a meses anteriores con un clic, sin amontonar todo el histórico subido en una sola línea ilegible. Funciones: `renderComboChartWithMonths()`, `renderComboMonthPills()`, `renderComboChartForSelectedMonth()`, `renderCharts()`.
...
- Ranking completo de vendedores (EFFI) y de tiendas (Dropi), con medallas 🥇🥈🥉 en el top 3. El ranking de tiendas queda oculto por completo para un visitante sin la clave (ver "Vista visitante bloqueada al mes actual").
- Tabla "Cumplimiento de vendedores" (meta de equipo versionada por periodo, ver "Topes / metas diarias") y tabla "Detalle por día" (con meta 200), con columnas EFFI/Dropi separadas **para el administrador**. Un visitante sin la clave ve "Detalle por día" con una sola columna "Unidades totales" en vez de dos. "Detalle por día" tiene además un gráfico (`renderDetalleChart()`) arriba de la tabla: barras EFFI+Dropi apiladas por día (o una sola barra "Unidades totales" para el visitante) con una línea punteada en la meta de 200, para ver de un vistazo qué días la cumplen — comparte el mismo selector de mes que la tabla (`detalleMesSelect`). Encima de cada columna se dibuja el total del día (EFFI+Dropi) con el mismo plugin `stackedTotalLabelPlugin` que usa "Historial y comparativo mensual". En modo "Resumen mensual" el gráfico muestra barras por mes sin la línea de meta ni el total encima (la meta es diaria, no mensual).
- **"Detalle por día" duplicado para el administrador (dos paneles en paralelo)**: cuando `adminUnlocked===true`, aparecen DOS paneles "Detalle por día" seguidos — "Detalle por dia (usuario)" (título en `#detalleTituloUsuario`, siempre combinado EFFI+Dropi en una columna/serie, igual que ve cualquier visitante) y "Detalle por dia (administrador)" (`#detallePanelAdmin`, siempre EFFI/Dropi separados). Un visitante sin clave sigue viendo un solo panel sin sufijo en el título. Implementado con `DETALLE_PANEL_CONFIG` (mapea `panelKey` 'usuario'/'admin' a sus propios ids de DOM: `tableHeadRow(Admin)`, `tableTh1(Admin)`, `tableTh4(Admin)`, `tableBody(Admin)`, `chartDetalle(Admin)`); `renderTableBody(panelKey)` y `renderDetalleChart(panelKey)` reemplazan a las versiones sin parámetro de antes, y `detalleCharts = {usuario:null, admin:null}` reemplaza a la variable única `detalleChart`. `renderTable(dates, effiByDate, dropiByDate)` no cambió de firma; llama a `renderTableBody('usuario')` siempre y a `renderTableBody('admin')` solo si `adminUnlocked`. Los dos paneles comparten un único selector de mes (`detalleMesSelect`, sin uno propio para el panel admin) — su listener `change` refresca ambos paneles a la vez. El panel nuevo usa el patrón `.admin-content` ya existente, con sus toggles agregados junto a los de `#adminContent`/`#historialSection` en `tryUnlockAdmin()` y en el listener de `adminLockBtn`.
...
- **KPIs exclusivos de administrador + tarjeta combinada para visitante**: "Unidades EFFI (mes)"
  (`kpiCardEffiMes`), "Unidades Dropi (mes)" (`kpiCardDropiMes`) y "Top tienda Dropi (mes)"
  (`kpiCardTopTienda`) se ocultan para un visitante sin la clave; en su lugar aparece siempre la
  tarjeta "Ventas totales (mes)" (`kpiVentasTotalesMes`, EFFI+Dropi del mes). El admin sigue viendo
  las 6 tarjetas originales, más esta séptima.
- **Ranking de tiendas oculto para visitante**: el panel "Ranking de tiendas" (`rankTiendaPanel`)
  se oculta por completo y `#rankGrid` pasa a una sola columna (`.rank-grid.single`) para un
  visitante sin la clave. El admin ve las dos columnas de siempre.
...
- **Dropi**: 20 unidades totales (24 filas − 4 canceladas, sin deduplicar). 2026-07-09 (jueves) → 1. 2026-07-10 (viernes) → 11. 2026-07-11 es **sábado**: 8 unidades crudas; como el lunes siguiente (07-13) **sí es festivo** (Día de Nuestra Señora del Rosario de Chiquinquirá, Ley 2578 de 2026), la ventana se extiende a sábado+domingo+lunes y se reparte entre 3 con `repartirEntero(8,3)` → **07-11 → 2**, **07-12 (domingo) → 3** y **07-13 (lunes festivo) → 3** (valores enteros exactos, suma=8, sin redondeo al mostrar). Tienda líder total (no cambia con el reparto): "Tiko" con 9 unidades.
...
- **KPI "Total combinado (último día)"**: el último día operativo con datos en esta muestra es 2026-07-13 (lunes festivo, generado por el reparto — antes de la Ley 2578 de 2026 hubiera sido 2026-07-12): EFFI 0 + Dropi 3 = **3 u.** exactos (faltan ~197 para la meta de 200) — distinto del combinado de 2026-07-10 (110 u.), que no es el último día.
